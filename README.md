...
- 部分设备可能不支持修改国家码
- 如需还原设置，请使用应用内的还原功能
- Android 16 / 部分 2025-12 之后安全补丁的系统，可能会直接拦截 `overrideConfig`，出现“保存失败”。这属于系统限制，不一定能通过普通应用侧完全绕过。

## 🔧 1.0.8 修复说明

- 修复部分 Android 9 / Android 10 设备因直接引用 `TelephonyFrameworkInitializer` 导致的启动闪退问题
- 改进 SIM 卡识别逻辑，优先基于活动订阅列表处理，改善双卡/部分机型识别异常
- 对无可用 SIM 卡场景给出更明确提示，避免 `phoneId/subId = -1` 一类报错直接暴露给用户
- 改进桌面端安装与版本检测逻辑，减少“已安装却检测异常”或安装失败信息不清的问题
- 将 fork 版本应用包名调整为 `com.cloudclawebot.nrfr`，避免与原版 `com.github.nrfr` 因签名不同而无法覆盖安装
- 修复读取电话权限后界面状态没有正确刷新，导致部分设备打开后 SIM 卡列表为空的问题
- 保存配置时新增 Android 16 fallback：持久化写入失败后，自动尝试非持久化写入
- 对新版系统上 `overrideConfig cannot be invoked by shell` 错误增加更明确的提示，避免只显示底层报错
